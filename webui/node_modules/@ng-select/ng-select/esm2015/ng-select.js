...
class ItemsList {
    ...
    filter(term) {
        ...
        for (const /** @type {?} */ key of Object.keys(this._groups)) {
            const /** @type {?} */ matchedItems = [];
            for (const /** @type {?} */ item of this._groups[key]) {
     ...
     */
    mapItem(item, index) {
        const /** @type {?} */ label = this.resolveNested(item, this._ngSelect.bindLabel);
        return {
            index: index,
    ...
    _groupBy(items, prop) {
        const /** @type {?} */ groups = items.reduce((grouped, item) => {
            const /** @type {?} */ key = item.value[prop];
    ...
    _flatten(groups) {
        let /** @type {?} */ i = 0;
        return Object.keys(groups).reduce((items, key) => {
            const /** @type {?} */ parent = {
                label: key,
                hasChildren: true,
                index: i,
            ...
            items.push(parent);
            i++;
            const /** @type {?} */ children = groups[key].map(x => {
                x.parent = parent;
                x.hasChildren = false;
                i++;
                return x;
